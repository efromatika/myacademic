...
const App = (() => {
  ...
  let filterSemester   = "all";   // filter halaman MK
  let scheduleSemester = "all";   // filter halaman Jadwal
  let searchQuery      = "";
  ...
  function renderSchedule() {
    // ── Filter MK berdasarkan semester ─────────────────────
    const filteredCourses = scheduleSemester === "all"
      ? courses
      : courses.filter(c => c.Semester?.toString() === scheduleSemester);

    // ── Filter JadwalExtra: tampilkan jika terkait MK di
    //    semester aktif, atau tidak terkait MK sama sekali
    const filteredMKIds = new Set(filteredCourses.map(c => c.ID?.toString()));
    const filteredExtra = scheduleSemester === "all"
      ? extraEvents
      : extraEvents.filter(ev =>
          !ev.MKId || filteredMKIds.has(ev.MKId?.toString())
        );

    // ── Update label & dropdown ────────────────────────────
    updateScheduleSemesterLabel();
    populateScheduleSemesterFilter();

    // ── Render grid ────────────────────────────────────────
    Scheduler.renderGrid("schedule-container", filteredCourses, filteredExtra, {
  ...
  function populateScheduleSemesterFilter() {
    const sel = document.getElementById("schedule-filter-semester");
    if (!sel) return;
    const sems = [...new Set(courses.map(c => c.Semester))]
      .filter(Boolean)
      .sort((a, b) => Number(a) - Number(b));
    // Jaga pilihan yang sedang aktif
    sel.innerHTML = `<option value="all">Semua Semester</option>` +
      sems.map(s =>
        `<option value="${s}" ${scheduleSemester === s.toString() ? "selected" : ""}>Semester ${s}</option>`
      ).join("");
  }

  function updateScheduleSemesterLabel() {
    const banner   = document.getElementById("schedule-semester-label");
    const labelEl  = document.getElementById("schedule-semester-label-text");
    if (!banner || !labelEl) return;
    if (scheduleSemester === "all") {
      banner.classList.add("hidden");
    } else {
      const semCourses = courses.filter(c => c.Semester?.toString() === scheduleSemester);
      const totalSKS   = semCourses.reduce((s, c) => s + Number(c.SKS || 0), 0);
      labelEl.textContent =
        `Menampilkan Semester ${scheduleSemester}  ·  ${semCourses.length} Mata Kuliah  ·  ${totalSKS} SKS`;
  ...
  function setupEventListeners() {
    ...
    // Filter semester jadwal
    document.getElementById("schedule-filter-semester")?.addEventListener("change", e => {
      scheduleSemester = e.target.value;
      renderSchedule();
    });

    // Tombol Print
    document.getElementById("btn-print-schedule")?.addEventListener("click", () => {
      if (typeof printSchedule === "function") printSchedule();
    });
